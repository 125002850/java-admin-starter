...
当前仓库已经具备 `admin-system/file` 文件存储能力，支持通过统一接口将文件写入 `local`、`qiniu`、`minio` provider，并提供删除、临时下载地址、直传凭证等能力。
...
- 复用现有 `admin-system/file` 能力完成文件落盘，不重复建设文件存储逻辑。
...
本设计遵循仓库顶层模块边界约定，不新增 `admin-export` 模块。
...
`admin-core` 只定义契约和抽象，不落导出记录表，不承载状态编排。

### 3.2 `admin-system`

继续承载统一文件存储能力。导出框架如需将渲染结果写入对象存储，必须通过 `admin-system/file` 提供的统一文件能力完成，不直接接触厂商 SDK。

### 3.3 `admin-mdm`

承载跨业务复用、带明确业务语义的平台型业务能力。本期文件导出中心及导出编排落在 `admin-mdm`，包括：
...
统一导出文件落盘抽象。首期由 `admin-system` 基于现有文件能力提供实现。
...
该抽象同样由 `admin-system` 基于现有文件能力提供实现，使 `admin-mdm` 无需直接依赖 `com.example.admin.file..`。
...
7. `FileStorageExportFileSink` 调用 `admin-system/file` 落盘
8. 平台回写 `SUCCESS` 或 `FAILED`

## 9. 下载中心接口设计

首期下载中心 HTTP 接口放在 `admin-mdm`。
...
- 通过现有 `admin-system/file` 获取临时下载地址
- 成功返回下载地址后，更新：
  - `download_count`
  - `last_download_time`
  - `last_download_by`
...
- 不新增 `admin-export` 顶层模块
- 不将文件存储能力上移到 `admin-core`
