package com.example.admin.core.query.dto;
...
class DynamicQueryDtoValidationTests {
    ...
    void paged_dynamic_query_should_inherit_default_page_values() {
        SamplePagedReqDTO reqDTO = new SamplePagedReqDTO();
    ...
    void paged_dynamic_query_should_limit_sort_items() {
        ...
        Set<ConstraintViolation<SamplePagedReqDTO>> violations = VALIDATOR.validate(reqDTO);

        assertThat(violations).isNotEmpty();
    }

    private SampleSortItem sort(SortField field) {
        SampleSortItem sortItem = new SampleSortItem();
    ...
    private static final class SampleConditionNode implements AbstractConditionNodeDTO {
    }

    private static final class SampleSortItem extends SortItemDTO<SortField> {
    }

    private static final class SampleCriteriaReqDTO extends BaseDynamicCriteriaReqDTO<SampleConditionNode, SampleSortItem> {
    }

    private static final class SamplePagedReqDTO extends BasePagedDynamicQueryReqDTO<SampleConditionNode, SampleSortItem> {
    }
}
