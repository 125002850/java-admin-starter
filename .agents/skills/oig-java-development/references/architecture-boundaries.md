...
| `admin-mdm` | 主数据、平台型业务服务和跨业务复用能力 | 当前承载全局字典与导出中心，不承载仅属于单一业务域的实现 |
| `admin-system` | 对象存储、SSO staff client 等外部系统集成 | 外部 SDK 只能出现在对应 provider 或 client 适配层 |
...
- 全局字典能力位于 `admin-mdm` 的 `mdm/dict` 包，接口统一暴露 `/api/mdm/dict/global/**`。
- 导出中心能力位于 `admin-mdm` 的 `mdm/export` 包，接口统一暴露 `/api/mdm/export/**`。
