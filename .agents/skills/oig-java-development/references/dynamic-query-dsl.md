...
新增的数据库分页查询默认使用动态查询 DSL。请求 DTO 继承 `BasePagedDynamicQueryReqDTO`，前端通过 `condition` 条件树传递筛选参数，禁止无业务理由地新增平铺查询字段。

已有公开 DTO 的兼容字段必须保持契约稳定，并在 scene mapper 中合并为 AST。对外部 API 的代理转发根据上游查询能力决定是否采用 DSL，不为形式统一伪造无法落地的筛选能力。
...
以上常量以 `DynamicQueryLimits` 和 `SceneQueryDefinition` 的当前实现为准；调整代码时同步更新本表和对应测试。
