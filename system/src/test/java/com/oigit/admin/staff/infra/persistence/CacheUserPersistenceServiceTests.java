package com.oigit.admin.staff.infra.persistence;
...
import com.oigit.admin.staff.infra.persistence.service.impl.CacheUserPersistenceServiceImpl;
...
class CacheUserPersistenceServiceTests {

    private CacheUserMapper cacheUserMapper;
    private CacheUserPersistenceServiceImpl cacheUserService;

    @BeforeEach
    void setUp() {
        cacheUserMapper = Mockito.mock(CacheUserMapper.class);
        cacheUserService = new CacheUserPersistenceServiceImpl(cacheUserMapper);
    }
    ...
    void should_resolve_usernames_for_non_empty_cached_users() {
        ...
        user101.setUserName(" ");
        when(cacheUserMapper.selectBatchIds(argThat(ids -> ids.contains(100L) && ids.contains(101L))))
    ...
    @Test
    void should_split_large_identifier_sets_into_fixed_size_queries() {
        when(cacheUserMapper.selectBatchIds(anyCollection())).thenReturn(List.of());
        List<Long> ids = LongStream.rangeClosed(1L, 501L).boxed().toList();

        cacheUserService.resolveUsernames(ids);

        verify(cacheUserMapper, times(2)).selectBatchIds(anyCollection());
    }
}
