...
import com.oigit.admin.iam.infra.config.IamProperties;
...
@Service
public class JwtService implements AccessTokenGateway {
    ...
    public void validateSecret() {
        iamProperties.assertSecretNotDefault();
    }

    public AccessToken issueAccessToken(Long staffId) {
        Instant issuedAt = Instant.now();
        Instant expiresAt =
                issuedAt.plusSeconds(Math.max(1, iamProperties.getAccessTokenTtlMinutes()) * 60);
    ...
    public JwtClaims parse(String token) {
        ...
        try {
            Map<String, Object> payload =
                    objectMapper.readValue(URL_DECODER.decode(parts[1]), MAP_TYPE);
    ...
    private String sign(String signingInput) {
        try {
            ...
            mac.init(
                    new SecretKeySpec(
                            iamProperties.getJwtSecret().getBytes(StandardCharsets.UTF_8),
                            "HmacSHA256"));
