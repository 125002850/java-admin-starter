package com.oigit.admin.iam.infra.request;
...
import com.oigit.admin.iam.infra.config.ClientIpProperties;
...
@Service
public class ClientRequestInfoResolver implements ClientRequestGateway, ClientIpResolver {
    ...
    public ClientRequestInfoResolver(ClientIpProperties properties) {
        trustedProxies =
                properties.getTrustedProxyCidrs().stream()
    ...
    public ClientRequestInfo resolve(HttpServletRequest request) {
        ...
        return new ClientRequestInfo(
                ip, truncate(request.getHeader("User-Agent"), MAX_USER_AGENT_LENGTH));
    }
    ...
    private List<String> forwardedAddresses(HttpServletRequest request) {
        ...
        return Arrays.stream(forwarded.split(",", -1)).map(this::extractForwardedFor).toList();
    }
    ...
    private String extractForwardedFor(String element) {
        for (String parameter : element.split(";", -1)) {
            ...
            if (separator < 0
                    || !parameter.substring(0, separator).trim().equalsIgnoreCase("for")) {
                continue;
            }
    ...
    private String stripForwardedPort(String value) {
        if (value.startsWith("[")) {
            ...
            if (bracket > 0
                    && (bracket == value.length() - 1 || value.charAt(bracket + 1) == ':')) {
        ...
        if (colon > 0
                && value.indexOf(':') == colon
                && value.substring(0, colon).contains(".")
