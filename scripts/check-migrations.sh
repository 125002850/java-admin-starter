...
STAGED_CHANGES="$(git diff --cached --name-status --find-renames -- "$MIGRATION_DIR" || true)"
...
if [ -n "$VIOLATIONS" ] || [ -n "$INVALID_NEW_MIGRATIONS" ] || [ -n "$DUPLICATE_MIGRATION_VERSIONS" ]; then
