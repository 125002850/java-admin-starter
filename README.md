...
推荐使用统一启动脚本。未配置远程数据源时，脚本会检查 Docker 是否运行；macOS 会自动启动 Docker Desktop，Linux 会尝试启动 `docker.service`，随后启动本地 MySQL 并等待健康检查通过：

```bash
# 1. 配置本地 JWT 密钥（至少 32 个随机字符）
export IAM_JWT_SECRET='<从本地安全配置读取>'

# 2. 可选：使用远程独立库 java_admin_starter_iam；配置后脚本会跳过 Docker
