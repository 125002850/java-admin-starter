...
import com.example.admin.iam.config.IamProperties;
import com.example.admin.iam.enums.IamErrorCode;
import com.example.admin.iam.infra.entity.IamRefreshTokenEntity;
import com.example.admin.iam.infra.mapper.IamRefreshTokenMapper;
...
public class RefreshTokenService {
    ...
    public IamRefreshTokenEntity validateForRefresh(String plainToken) {
        ...
        IamRefreshTokenEntity entity = refreshTokenMapper.selectOne(
                Wrappers.<IamRefreshTokenEntity>lambdaQuery()
                        .eq(IamRefreshTokenEntity::getTokenHash, tokenHash)
                        .last("limit 1")
        );
        LocalDateTime now = LocalDateTime.now();
    ...
    public IssuedRefreshToken rotate(IamRefreshTokenEntity oldToken) {
        int updated = revokeIfActive(oldToken, "ROTATED");
        if (updated == 0) {
            throw new BizException(IamErrorCode.AUTH_REFRESH_TOKEN_INVALID);
        }
    ...
    private int revokeIfActive(IamRefreshTokenEntity entity, String reason) {
        LocalDateTime now = LocalDateTime.now();
        LambdaUpdateWrapper<IamRefreshTokenEntity> update = Wrappers.<IamRefreshTokenEntity>lambdaUpdate()
                .eq(IamRefreshTokenEntity::getId, entity.getId())
                .isNull(IamRefreshTokenEntity::getRevokedTime)
                .set(IamRefreshTokenEntity::getRevokedTime, now)
                .set(IamRefreshTokenEntity::getRevokeReason, reason);
        return refreshTokenMapper.update(update);
    }
